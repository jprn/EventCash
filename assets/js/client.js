  ...
  async function renderPayQr(token){
    ...
    try{
      ...
      if(window.QRCode && typeof window.QRCode.toCanvas === 'function'){
        ...
        await window.QRCode.toCanvas(canvas,String(token),{width:280,margin:2,errorCorrectionLevel:'H',color:{dark:'#000000',light:'#ffffff'}});
        return;
      }
    }catch(_e){
      // fallback
    ...
    img.src='https://api.qrserver.com/v1/create-qr-code/?size=280x280&color=0-0-0&bgcolor=255-255-255&margin=2&ecc=H&data='+encodeURIComponent(String(token));
    el.appendChild(img);
  }
